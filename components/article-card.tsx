...
export function ArticleCard({ article }: { article: ResearchArticle }) {
  ...
  const content = (
    <article className="grid gap-5 border-b border-line py-8 transition-colors hover:border-line-strong md:grid-cols-[0.28fr_0.72fr]">
      ...
      <div>
        <div className="flex flex-wrap items-center gap-3">
          <h2 className="font-serif text-3xl font-medium leading-tight">{article.title}</h2>
          <span className="border border-line px-2 py-1 font-mono text-xs text-subtle">
            {article.status}
          </span>
        </div>
        <p className="mt-4 max-w-3xl text-sm leading-6 text-muted">{article.summary}</p>
